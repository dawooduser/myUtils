...
export const HomeScreen = ({route}: any) => {
  ...
  const handlePrint = async () => {
    try {
      ...
      //   cut: false,
      // });
      // Printer.printBill('<C>check this ou</C>');
      const BOLD_ON = COMMANDS.TEXT_FORMAT.TXT_BOLD_ON;
    const BOLD_OFF = COMMANDS.TEXT_FORMAT.TXT_BOLD_OFF;
    const CENTER = COMMANDS.TEXT_FORMAT.TXT_ALIGN_CT;
    const OFF_CENTER = COMMANDS.TEXT_FORMAT.TXT_ALIGN_LT;
    Printer.printText(`${CENTER}Salam Taha Salman ABC`);
    Printer.printColumnsText(['xx/xx/xx', '+923357195498'], [10, 10], 
    [ColumnAlignment.RIGHT, ColumnAlignment.LEFT], [`${BOLD_OFF}`, '', ''])
